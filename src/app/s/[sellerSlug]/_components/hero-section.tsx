...
export function HeroSection({ section, seller, sellerSlug, isDealer = false, isStaff = false }: HeroSectionProps) {
  const base = "";
  return (
    <section className="relative overflow-hidden border-b border-border bg-card dark:bg-background">
      {/* Gradient background overlay */}
      <div className="absolute inset-0 bg-gradient-to-br from-blue-50/80 via-transparent to-red-50/50 dark:from-blue-950/30 dark:via-transparent dark:to-red-950/20" />
      ...
      <div className="site-container relative grid min-h-[520px] items-center gap-8 py-14 lg:grid-cols-[.9fr_1.1fr] lg:py-10">
        <div className="relative z-10 max-w-2xl animate-fade-in">
          ...
          <h1 className="text-4xl font-black leading-[1.05] tracking-tight text-foreground md:text-6xl">
            ...
            <span className="bg-gradient-to-r from-red-600 to-red-500 bg-clip-text text-transparent dark:from-red-400 dark:to-red-500">
          ...
          <div className="mt-7 flex flex-wrap gap-3">
            ...
            {isDealer ? (
              <Link
                ...
                className="inline-flex items-center gap-2 rounded-xl border border-border bg-card px-6 py-3.5 text-sm font-extrabold text-foreground hover:bg-accent transition-all glass-card"
              >
          ...
          <div className="mt-8 grid max-w-xl grid-cols-2 gap-2 sm:grid-cols-4 stagger-children">
            {[ [Boxes,"Complete catalogue","text-blue-500"], [FileCheck2,"Proforma workflow","text-emerald-500"], [PackageCheck,"Warehouse picking","text-violet-500"], [Truck,"Dispatch tracking","text-amber-500"] ].map(([Icon, label, color]) => {
              ...
              return (
                <div key={label as string} className="flex items-center gap-2 rounded-xl border border-border bg-card/80 backdrop-blur-sm px-3 py-2.5 text-[10px] font-bold text-foreground shadow-sm transition-all hover:-translate-y-0.5 hover:shadow-md">
        ...
        <div className="relative min-h-[340px] lg:min-h-[500px]">
          ...
          />
          <div className="absolute bottom-3 right-0 w-[210px] glass-card p-4 sm:w-[240px] lg:bottom-10">
