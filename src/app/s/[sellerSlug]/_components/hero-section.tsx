    ...
    <section className="relative overflow-hidden bg-[#0A0A0B] dark:bg-[#050505] min-h-[700px] flex items-center">
      ...
      <div className="site-container relative z-10 grid min-h-[520px] items-center gap-12 py-20 lg:grid-cols-[1fr_1fr] lg:py-16">
        <div className="relative z-10 max-w-2xl animate-in slide-in-from-bottom-8 duration-700 fade-in fill-mode-both">
          <div className="inline-flex items-center gap-2 rounded-full border border-red-500/30 bg-red-950 px-4 py-1.5 text-sm font-semibold text-red-400 mb-6">
          ...
          <div className="mt-10 flex flex-wrap gap-4">
            <Link
              href={`${base}/products`}
              className="group relative inline-flex items-center gap-2 rounded-full bg-gradient-to-r from-red-600 to-red-500 px-8 py-4 text-sm font-bold text-white shadow-[0_0_40px_-10px_rgba(220,38,38,0.5)] transition-all hover:scale-105 hover:shadow-[0_0_60px_-15px_rgba(220,38,38,0.7)] border-none"
            >
              Explore Catalogue 
            ...
            {isDealer ? (
              <Link
                ...
                className="group inline-flex items-center gap-2 rounded-full bg-slate-800 px-8 py-4 text-sm font-bold text-white transition-all hover:bg-slate-700 border-none"
              >
          ...
          <div className="mt-14 grid max-w-xl grid-cols-2 gap-4 sm:grid-cols-4 animate-in slide-in-from-bottom-8 duration-700 delay-200 fade-in fill-mode-both">
            {[ [Boxes,"Complete Catalogue","text-blue-400", "bg-blue-950"], [FileCheck2,"Proforma Workflow","text-emerald-400", "bg-emerald-950"], [PackageCheck,"Warehouse Picking","text-violet-400", "bg-violet-950"], [Truck,"Dispatch Tracking","text-amber-400", "bg-amber-950"] ].map(([Icon, label, color, bg]) => {
              const ItemIcon = Icon as React.ElementType;
              return (
                <div key={label as string} className="flex flex-col items-center justify-center gap-3 rounded-2xl bg-slate-900 border-none p-4 text-center transition-all hover:-translate-y-1 hover:bg-slate-800">
        ...
        <div className="relative min-h-[400px] lg:min-h-[600px] flex items-center justify-center animate-in zoom-in-95 duration-1000 delay-300 fade-in fill-mode-both">
          ...
          {/* Solid Floating Card */}
          <div className="absolute bottom-10 right-4 lg:-right-4 w-[280px] rounded-2xl bg-slate-900 border-none p-5 shadow-2xl z-20 transition-transform hover:-translate-y-2 duration-300">
            <div className="mb-4 flex items-center justify-between border-b border-slate-800 pb-4">
              ...
              <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-red-950">
            ...
            {isDealer ? (
              <Link
                href="/dealer/dashboard"
                className="group mt-5 flex items-center justify-between rounded-xl bg-slate-800 border-none px-4 py-3 text-xs font-extrabold text-white hover:bg-slate-700 transition-all"
              >
